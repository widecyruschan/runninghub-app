...
> 更新日期：2026-07-12  
> 当前状态：已完成 SQLite、工具配置保存 API、后台新增/编辑工具保存、后台分类管理基础、公开工具 API、前台动态工具页基础、统一工具执行 API、本地任务记录基础、后台工具测试执行基础、前台工具市场首页基础、后台任务列表基础。
...
- 前台已可通过 `/` 展示数据库驱动的工具市场首页，并通过 `/tools/:slug` 进入工具执行页。
- 公开工具 API 已完成基础版本，尚未支持关键词、分类、热门等查询参数。
- 统一工具执行 API 已完成基础版，后续需要补充更多文件类型、队列和鉴权。
- 已新增本地 `execution_tasks` 任务表，并已接入后台任务列表基础；后续需要接入前台用户中心执行历史。
- 后台工具上线前已可发起测试执行，后续需要补充更完整的测试历史列表。
- 前台已完成基础分类筛选、搜索和全部工具列表，首页热门工具、服务端查询参数和排序权重后续再补。
...
- 已完成 `/` 前台工具市场首页。
- 首页已读取 `GET /api/tools` 和 `GET /api/categories`。
- 首页已支持：
  - 全部工具列表
  - 分类筛选
  - 关键词搜索
  - 工具预览图卡片
  - 点击卡片进入 `/tools/:slug`
- `/tools/:slug` 继续复用动态工具表单、统一执行 API、本地任务轮询和输出查询。
- 后续 `tools` 表继续增加：
  - `is_featured`
  - `tags_json`
  - `credit_cost`
...
- 前台中英双语：
  - `zh-HK`
  - `en-US`
- 后台前台内容录入采用“中文主内容 + 英文译文”的双栏结构，英文缺失时前台先回退中文并在后台标记缺失。
- 封装 `translationProvider`：
  - 默认 provider：Azure Translator。
  - 预留 provider：Google Cloud Translation、LibreTranslate。
  - Provider 层只负责翻译文本，不直接处理工具保存、文章保存或页面渲染。
  - 后台在运营人员点击“自动翻译”或保存草稿时生成英文译文。
  - 英文译文必须允许人工修改，避免机器翻译直接覆盖已校对内容。
- 翻译接入顺序：
  1. 先完成本地 i18n 数据结构和前台语言切换。
  2. 再接 `translationProvider` 抽象和 Azure 实现。
  3. 最后按需要补 Google / LibreTranslate 适配器。
- 错误码多语言字典。
...
- 每个上线工具有可索引页面。
- 中文和英文页面 URL 清晰。
- 后台输入中文后可以自动生成英文草稿，且人工修改后的英文不会被静默覆盖。
- 翻译 provider 可通过配置切换，业务层不依赖具体供应商。
- 常见流程有自动化测试。
...
- 后台已经能查看任务记录，下一步要让任务真正归属到前台用户。
- 积分扣费必须建立在任务记录和用户历史之上，不能只改余额。
...
- 套餐权益依赖用户、积分和任务记录。
- 权益控制做好后，再进入批量、云盘和推荐奖励会更稳。

### 第三迭代：前台 I18N 与自动翻译准备

交付范围：

- 前台语言切换和本地语言偏好。
- 工具、分类、表单字段、富文本说明的中英文数据结构。
- 后台英文缺失提示。
- `translationProvider` 抽象设计，默认 Azure Translator，预留 Google Cloud Translation 和 LibreTranslate。

为什么第三做：

- 基础工具执行、任务排查和用户地基优先级更高。
- I18N 需要改动前台展示字段、后台录入字段和数据结构，适合在核心闭环稳定后集中推进。
