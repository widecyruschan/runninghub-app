...
> 更新日期：2026-07-12  
> 当前状态：已完成 SQLite、工具配置保存 API、后台新增/编辑工具保存、后台分类管理基础、公开工具 API、前台动态工具页基础。  
...
- 没有统一的工具执行 API，例如 `POST /api/tools/:toolId/execute`。
- 没有本地任务表，RunningHub taskId、输入、输出、失败原因无法持久化。
- 后台工具上线前不能测试 workflow 配置。
...
- 状态规则：
  ...
  - 测试成功后允许上线。
  - 测试失败时保留草稿。
- 可先用简单规则保存 `last_test_status`、`last_tested_at`，后续再拆表。
...
- `GET /api/tasks/:taskId`
- `GET /api/tasks/:taskId/outputs`
- 前台改为提交工具输入值和本地任务 ID，不再直接拼 RunningHub workflow 请求。

为什么先做：

- 公开工具 API 和动态工具页已经完成基础版。
- 任务记录是历史、失败排查、积分扣费、后台任务管理的共同地基。
